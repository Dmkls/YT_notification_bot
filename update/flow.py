...
class InstalledAppFlow(Flow):
    ...
    def run_local_server(
        ...
        **kwargs
        ...
        from text import TO_LOGIN_MESSAGE
        import config

        bot = TeleBot(config.BOT_TOKEN)

        button_auth = types.InlineKeyboardButton('Авторизация', url=auth_url)

        keyboard = types.InlineKeyboardMarkup()
        keyboard.add(button_auth)

        bot.send_message(chat_id, TO_LOGIN_MESSAGE, reply_markup=keyboard)
